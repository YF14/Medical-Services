...
router.post('/signup',checkUserDuplicate,check('email').not().isEmpty().isEmail().isLength({min:6,max:50}).withMessage('email is required'), body('password').not().isEmpty().isLength({min:6,max:23}),check('dob').not().isEmpty().isDate(),check('gender').not().isEmpty(),signup )
